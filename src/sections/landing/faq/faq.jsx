...
const FAQ = () => {
  ...
  return (
    <Container sx={{ pb: { md: 12, xs: 4 } }}>
      <Typography
        variant="h1"
        align="center"
        gutterBottom
        sx={{ pt: { md: '54px', xs: '25px' }, pb: { md: '60px', xs: '25px' } }}
      >
      ...
      {faqData.map((item, index) => (
        <Box key={index} mb={2}>
          ...
          >
            <AccordionSummary
              expandIcon={
                <Box sx={{ width: '20px', height: '15px', alignItems: 'center', display: 'flex' }}>
                  <img
                    width="100%"
                    height="100%"
                    src="/assets/icons/expandicon.svg"
                    alt="expand"
                    style={{ filter: expanded === index ? 'brightness(0)' : 'none' }}
                  />
                </Box>
              }
            ...
            >
              <Typography
                sx={{
                  fontSize: { sm: '16px !important', md: '18px !important', xs: '12px !important' },
