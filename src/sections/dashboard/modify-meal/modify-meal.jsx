...
import { useNavigate, useSearchParams } from 'react-router-dom'; // Use useSearchParams for search params
import { Box, Button, Dialog, Tooltip, Typography, DialogContent } from '@mui/material';

import Calendar from './modify-calendar';
import Iconify from '../../../components/iconify';
import MealPlanningHead from './meal-planning-head';
import PreferencesDialog from './saved-preferences';

const ModifyMeal = () => {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [isPreferencesDialogOpen, setPreferencesDialogOpen] = useState(false);
  const [searchParams, setSearchParams] = useSearchParams(); // Hook for handling search params
  const [activeComponent, setActiveComponent] = useState(
    searchParams.get('activeTab') || 'detailed'
  ); // Default to 'detailed' if no search param

  useEffect(() => {
    if (searchParams.get('activeTab')) {
      setActiveComponent(searchParams.get('activeTab')); // Update activeComponent based on searchParams
    }
  }, [searchParams]);
  ...
  // Update search params when switching components
  const handleComponentSwitch = (component) => {
    setSearchParams({ activeTab: component });
      ...
      <Box pt="20px">
        ...
        <Box sx={{ display: 'flex', pt: '20px' }}>
          <Box sx={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <Button
              variant={activeComponent === 'detailed' ? 'contained' : 'outlined'} // Conditionally set variant
              sx={{
                color: activeComponent === 'detailed' ? '#fff' : '#000', // Text color based on variant
              }}
              onClick={() => handleComponentSwitch('detailed')} // Set active component and update search params
            >
              Detailed version
            </Button>
            <Button
              variant={activeComponent === 'calendar' ? 'contained' : 'outlined'} // Conditionally set variant
              sx={{
                color: activeComponent === 'calendar' ? '#fff' : '#000', // Text color based on variant
              }}
              onClick={() => handleComponentSwitch('calendar')} // Set active component and update search params
            >
              Calendar version
            </Button>
          </Box>
        ...
        <Dialog open={open} onClose={handleDialogClose}>
          <DialogContent>
            ...
            <Typography>
              Are you sure you want to leave this section without saving the meal plan? If you do
              so, all modifications will be lost.
            </Typography>
          </DialogContent>
        ...
        {/* Render Calendar or Detailed View */}
        {activeComponent === 'detailed' ? <MealPlanningHead /> : <Calendar />}
